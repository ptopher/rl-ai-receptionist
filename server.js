...
function getSpokenMachineLabel(input, internalMachine) {
  const rawCleaned = cleanText(input);

  if (rawCleaned.includes('lawn tractor') || rawCleaned.includes('tractor mower') || rawCleaned.includes('my tractor') || rawCleaned.includes('the tractor') || rawCleaned === 'tractor') {
    return 'lawn tractor';
  }

  if (rawCleaned.includes('push mower') || rawCleaned.includes('walk behind') || rawCleaned.includes('walk-behind')) {
    return 'push mower';
  }

  if (rawCleaned.includes('self propelled') || rawCleaned.includes('self-propelled')) {
    return 'self-propelled mower';
  }

  if (rawCleaned.includes('lawn mower') || rawCleaned.includes('lawnmower')) {
    return 'lawnmower';
  }

  if (rawCleaned.includes('riding mower') || rawCleaned.includes('ride on mower') || rawCleaned.includes('rider mower')) {
    return 'riding mower';
  }

  return String(internalMachine || 'machine').toLowerCase();
}
      ...
      switch (data.type) {
        ...
        case 'prompt': {
          ...
          if (!callState.machine) {
            ...
            if (m) {
              ...
              if (!callState.machineSpoken) {
                callState.machineSpoken = getSpokenMachineLabel(userText, m);
              }
          ...
          if (!callState.inScheduling && !callState.callerName) {
            ...
            if (isEquipmentQuestion && !callState.machine) {
              ...
              if (detected) {
                ...
                if (!callState.machineSpoken) {
                  callState.machineSpoken = getSpokenMachineLabel(userText, detected);
                }
          ...
          if (callState.machine && callState.issue && !callState.askedForSchedule && !callState.inScheduling) {
            callState.askedForSchedule = true;
            await emmaReply(userText,
              `Issue is noted. Ask if they want to schedule an appointment without repeating the machine again.`,
              `Got it. Would you like to schedule a time for us to come take a look?`);
            break;
          }
